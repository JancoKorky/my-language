...
then u can use lambda functions in If statement
```
ak(
    rovnaju_sa(1,1),
    () => vypis("true"),
...
```
##### IF
If statement is function with structure like this:
`ak(condition_function, then_function, else_function)`
so how you could see before you can use it like this:
```
ak(
    rovnaju_sa(1,1),
    () => vypis("true"),
...
## Build-in functions (runtime functions)
You can see all of them in file `runtime.js`.
But there is list of them:
```
function vypis(...args) {
  console.log(...args);
}
